...
function createButtonSlot(kind, entryIndex, enabled = true) {
    ...
    slot.appendChild(el('span', { className: 'button-slot-text', style: 'display: none; font-size: 11px; font-weight: bold; padding: 0 4px;' }, [document.createTextNode('Mined')]));
...
function applyButtonSlotVisualState(slot) {
    ...
    const iconSpan = slot.querySelector('.button-slot-icon');
    const textSpan = slot.querySelector('.button-slot-text');
    
    if (kind === 'mine' && state === 'duplicate') {
        if (iconSpan) iconSpan.style.display = 'none';
        if (textSpan) textSpan.style.display = 'inline-block';
        slot.classList.add('is-mined');
        slot.disabled = true;
    } else {
        if (iconSpan) iconSpan.style.display = 'block';
        if (textSpan) textSpan.style.display = 'none';
        slot.classList.remove('is-mined');
