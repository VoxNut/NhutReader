...
import com.nhut.hoshi.features.audio.LocalAudioFile
import com.nhut.hoshi.features.audio.LocalAudioRepository
import com.nhut.hoshi.features.audio.LocalAudioResolver
...
internal class AnkiRepository(
    ...
    suspend fun mineEntry(
        ...
        val finalFields = if (settings.aiTranslateSentence && settings.aiTranslateField.isNotBlank()) {
            val translated = runCatching {
                val translationSettingsRepository = com.nhut.hoshi.features.translation.TranslationSettingsRepository(
                    this@AnkiRepository.context.translationSettingsDataStore
                )
                val translationSettings = translationSettingsRepository.settings.first()
                val geminiService = com.nhut.hoshi.features.translation.GeminiTranslationService()
                val result = geminiService.translate(
                    text = context.sentence,
                    contextSentence = "",
                    sourceLanguage = "Japanese",
                    targetLanguage = "English",
                    userApiKey = translationSettings.geminiApiKey,
                    ankiKeyword = payload.expression
                )
                result.translatedText
            }.getOrElse { "" }
            fields + (settings.aiTranslateField to translated)
        } else {
            fields
        }
        ...
        if (added) {
            ...
            runCatching {
                com.nhut.hoshi.features.firebase.HoshiFirebaseManager.saveAnkiMining(
                    word = payload.expression,
                    reading = payload.reading,
                    sentence = context.sentence,
                    meaning = payload.glossary,
                    deck = deck.name,
    ...
    suspend fun isDuplicate(
        ...
        val activeBackend = activeBackendOrError(settings).getOrElse {
            Log.d("AnkiRepository", "isDuplicate: activeBackendOrError failed")
            return@withContext false
        }
        val availableNoteTypes = noteTypes.ifEmpty { activeBackend.fetchNoteTypes() }
        val availableDecks = decks.ifEmpty { activeBackend.fetchDecks() }
        val deck = availableDecks.firstOrNull { it.id == settings.selectedDeckId }
            ?: settings.selectedDeckName?.let { name -> availableDecks.firstOrNull { it.name == name } }
            ?: run {
                Log.d("AnkiRepository", "isDuplicate: deck not found for id=${settings.selectedDeckId}, name=${settings.selectedDeckName}")
                return@withContext false
            }
        val noteTypeId = settings.selectedNoteTypeId
            ...
            ?: run {
                Log.d("AnkiRepository", "isDuplicate: noteTypeId not found")
                return@withContext false
            }
        val noteType = availableNoteTypes.firstOrNull { it.id == noteTypeId } ?: run {
            Log.d("AnkiRepository", "isDuplicate: noteType not found for id=$noteTypeId")
            return@withContext false
        }
        Log.d("AnkiRepository", "isDuplicate: calling activeBackend.isDuplicate for expression='$expression'")
