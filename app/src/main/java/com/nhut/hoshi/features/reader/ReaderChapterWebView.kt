...
internal fun ChapterWebView(
    ...
    val currentOnTranslateText = rememberUpdatedState(onTranslateText)
    ...
    AndroidView(
        ...
        factory = { context ->
            HoshiReaderWebView(context).apply {
                ...
                this.onTranslateText = { text ->
                    currentOnTranslateText.value(text)
                }
        ...
        update = { webView ->
            ...
            fun shouldIgnoreReaderGestureEvent(event: MotionEvent): Boolean {
                if (currentIsWebViewRestoring.value || webView.isSelectionActive()) {
                    return true
                }
                val density = webView.resources.displayMetrics.density
...
internal class HoshiReaderWebView(context: Context) : WebView(context) {
    var onHighlightCreated: (HighlightColor, String, ReaderHighlightCreationResult) -> Unit = { _, _, _ -> }
    var onTranslateText: ((String) -> Unit)? = null

    fun translateNativeSelection() {
        evaluateJavascript(
            "(function() { return window.getSelection().toString(); })()",
        ) { textJson ->
            Log.d("HoshiReaderWebView", "Raw text selection JSON: $textJson")
            val text = runCatching {
                if (textJson != null && textJson != "null") {
                    val array = org.json.JSONArray("[$textJson]")
                    array.optString(0, "")
                } else ""
            }.onFailure { e ->
                Log.e("HoshiReaderWebView", "Failed to parse text selection JSON: $textJson", e)
            }.getOrDefault("").trim()
            Log.d("HoshiReaderWebView", "Parsed selection text: '$text'")
            if (text.isNotEmpty()) {
                onTranslateText?.invoke(text)
            }
        }
    }
    private var nativeSelectionActionModeActive = false
    private var isJavascriptSelectionActive = false
    ...
    fun isSelectionActive(): Boolean = nativeSelectionActionModeActive || isJavascriptSelectionActive
    
    fun setJavascriptSelectionActive(active: Boolean) {
        isJavascriptSelectionActive = active
    }
...
private class ReaderHighlightActionModeCallback(
    ...
    override fun onActionItemClicked(mode: ActionMode, item: MenuItem): Boolean {
        ...
        if (item.itemId == ReaderHighlightSelectionMenu.translateItemId) {
            webView.translateNativeSelection()
            mode.finish()
            return true
        }
    ...
    private fun addHighlightMenu(menu: Menu) {
        if (menu.findItem(ReaderHighlightSelectionMenu.translateItemId) == null) {
            menu.add(
                ReaderHighlightSelectionMenu.groupId,
                ReaderHighlightSelectionMenu.translateItemId,
                Menu.NONE,
                webView.context.getString(R.string.reader_translate_action),
