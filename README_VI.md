...
- **Nhập sách linh hoạt**: Hỗ trợ nhập tệp EPUB riêng lẻ, nhập hàng loạt, hoặc quét tự động đệ quy từ các thư mục.
- **Tiến trình trực quan**: Hiển thị phần trăm tiến độ đọc trực tiếp trên ảnh bìa sách.
- **Phân loại kệ sách**: Tổ chức thư viện cá nhân bằng các kệ sách tự tạo.
- **Đồng bộ đám mây**: Xuất file EPUB hoặc tải sách đã đồng bộ từ máy chủ lưu trữ từ xa về máy.

### Trải Nghiệm Đọc Sách Chuyên Sâu
- **Định dạng hiển thị**: Hỗ trợ đọc văn bản tiếng Nhật theo chiều dọc (truyền thống) hoặc chiều ngang, với chế độ cuộn liên tục hoặc phân trang.
- **Cấu hình giao diện**: Tùy chỉnh khoảng cách lề, khoảng cách dòng, phông chữ hệ thống/phông chữ tùy chỉnh, và các chủ đề đọc sách đa dạng (Sáng, Tối, Sepia).
- **Phím cứng & Tiết kiệm năng lượng**: Hỗ trợ chuyển trang bằng phím âm lượng và tối ưu hóa hiển thị giảm tần số quét cho màn hình E-ink.
- **Xem ảnh toàn màn hình**: Phóng to hình ảnh trong sách, sao chép, lưu, hoặc chia sẻ trực tiếp.

### Tra Cứu Từ Điển Yomitan Nâng Cao
- **Quản lý dữ liệu từ điển**: Tải xuống, nhập, cập nhật, và bật/tắt các bộ từ điển Yomitan định dạng ZIP trực tiếp trên thiết bị.
- **Tra cứu đa phương thức**: Chạm trực tiếp để tra từ khi đang đọc, tìm kiếm thủ công trong tab Từ điển, hoặc chia sẻ văn bản bôi đen từ ứng dụng khác sang Nhut Reader để tra cứu.
- **Tra cứu đệ quy**: Nhấp vào các từ chưa biết trong phần giải thích nghĩa của từ điển để tiếp tục tra cứu đa tầng.
- **Tùy biến phong cách**: Nhúng CSS tùy chỉnh cho popup nghĩa, chọn nguồn phát âm thanh từ vựng trực tuyến hoặc cục bộ.

### Dịch Thuật Bằng Trí Tuệ Nhân Tạo (AI)
- **Tích hợp Gemini API**: Dịch từ vựng hoặc cả câu trực tiếp từ sách hoặc popup từ điển bằng các mô hình AI tiên tiến (như `gemini-2.5-flash`, `gemini-2.5-pro`, hoặc `gemini-3-flash-preview`).
- **Dịch tự động**: Tự động kích hoạt dịch khi chọn từ hoặc câu.
- **Đồng bộ thẻ Anki**: Tự động định dạng câu dịch khi tạo thẻ Anki để làm nổi bật từ khóa chính (ví dụ: bọc câu trong lớp `<span class="group">...</span>` và từ khóa chính trong lớp `<span class="highlight">...</span>`).
- **Nhật ký dịch thuật**: Tự động sao lưu lịch sử dịch thuật lên Cloud Firestore (Firebase).

### Đánh Dấu & Thống Kê Học Tập
- **Highlight nhiều màu**: Đánh dấu văn bản với 5 màu khác nhau, dễ dàng chuyển nhanh đến các đoạn đã đánh dấu.
- **Thống kê thời gian thực**: Đo lường số ký tự đã đọc, thời gian đọc, và tốc độ đọc trung bình, hiển thị ngay trên thanh công cụ khi đang đọc.

### Tạo Thẻ Từ Vựng Anki (Mining)
- **Hỗ trợ đa nền tảng**: Tạo thẻ tức thời qua ứng dụng khách AnkiDroid cục bộ hoặc máy chủ AnkiConnect.
- **Ánh xạ trường thông minh**: Tự động điền dữ liệu theo mẫu trường tùy chỉnh (tương thích với định dạng Lapis).
- **Kiểm tra trùng lặp tự động**: Tránh tạo thẻ trùng lặp bằng cách truy vấn mã checksum đã ký và chưa ký trên toàn bộ bộ sưu tập hoặc deck cụ thể.
- **Nhật ký tạo thẻ**: Quản lý lịch sử các từ đã mine kèm thời gian tạo cụ thể trong tab Lịch sử Mine từ.

### Nghe Sách Nói Đồng Bộ (Sasayaki)
- **Đồng bộ âm thanh & chữ**: Khớp tệp phụ đề (SRT/WebVTT) của sách nói với văn bản EPUB để tự động làm nổi bật câu đang phát.
- **Tự động chuyển trang**: Tự động cuộn trang hoặc lật trang sách để khớp với tốc độ đọc của âm thanh.
- **Trình điều khiển thông minh**: Điều chỉnh tốc độ phát, tua nhanh/tua lại câu, liên kết trực tiếp với thanh thông báo hệ thống của Android (MediaSession).
